...
contract Deed {
    ...
    function setBalance(uint newValue) onlyRegistrar onlyActive payable { 
        ...
        if (this.balance < newValue) throw; // this.balance is the amount sent with the original bid
...
contract Registrar {
    ...
    mapping (bytes32 => entry) public entries; // maps hashed names to an entry (line 116), which maps to a deed. 
    mapping (bytes32 => Deed) public sealedBids; //maps sealed bid hashes to deeds
    ...
    function newBid(bytes32 sealedBid) payable {
        ...
        if (!newBid.send(msg.value)) throw; //send the amount received to the bid Deed
    } 
    ...
    function unsealBid(bytes32 _hash, address _owner, uint _value, bytes32 _salt) {
        ...
        */
        if (bid.creationDate() > h.registrationDate - revealPeriod // bid was created after the reveal period began
            || now > h.registrationDate // now is after created the registration date
            || _value < minPrice) {
            ...
            h.highestBid = _value;
            h.deed = bid;
            bid.setBalance(_value); // this will throw if _value is less than the amount submitted, so such bids can never be revealed. 
    ...
    function cancelBid(bytes32 seal) {
        ...
        // You get back less for cancelling an old bid than revealing it... this is probably to incentivize not polluting the list of bids
    ...
    /**
     * @dev Finalize an auction. After the registration date has passed, calling this 
     * function will call the ENS's setSubnodeOwner function to assign the ownership of
     * the subnode to the winning bidder.
